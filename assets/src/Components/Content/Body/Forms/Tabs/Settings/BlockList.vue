<template>
	<div class="w-full flex flex-col border   rounded-lg my-2">
		...
		<div class=" px-4 py-2  flex flex-row flex-wrap ">
		...
		<div class="flex flex-row justify-end py-2 px-4">
				<button @click="updateList" class="px-4 py-2 border rounded flex flex-row items-center"><svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-1" viewBox="0 0 20 20" fill="currentColor"> <path d="M7.707 10.293a1 1 0 10-1.414 1.414l3 3a1 1 0 001.414 0l3-3a1 1 0 00-1.414-1.414L11 11.586V6h5a2 2 0 012 2v7a2 2 0 01-2 2H4a2 2 0 01-2-2V8a2 2 0 012-2h5v5.586l-1.293-1.293zM9 4a1 1 0 012 0v2H9V4z" /> </svg>Save</button>
		</div>
	</div>
</template>
...
function updateList() {
	...
	if (typeof blockList.value != 'object') return;

	
	const data = new FormData();
	data.append('awraq_nonce', awraq_nonce);
	data.append('action', 'awraqUpdateBlockedIps');
	data.append('ipList', JSON.stringify(blockList.value));
...
function validateIpAddress(ipaddress) {
	/**
	 * i dont know how this regex works, i just copied from stackoverflow
	 */
...
function addIpToList() {
	let validatedIp = validateIpAddress(tempIp.value);
	if (validatedIp != false) {
		blockList.value.push(validatedIp);
	}
