<template>
  ...
  <div v-if="categories.length === 0 "  class="container mx-auto md:w-7/12 py-10 flex flex-col items-center">
    <h2 class="flex items-center text-2xl font-semibold text-gray-600 tracking-wide mb-4">No Product Categories.</h2>
    <p class="text-md font-semibold text-gray-500"> Create product Categories by clicking the button below.</p>
    <a href="/wp-admin/edit-tags.php?taxonomy=product_cat&post_type=product" target="_blank" class="bg-white border px-4 py-2 rounded-full text-gray-500 font-semibold capitalize mt-4"><svg xmlns="http://www.w3.org/2000/svg" class="inline fill-current w-5 h-5 mr-1" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M17.707 9.293a1 1 0 010 1.414l-7 7a1 1 0 01-1.414 0l-7-7A.997.997 0 012 10V5a3 3 0 013-3h5c.256 0 .512.098.707.293l7 7zM5 6a1 1 0 100-2 1 1 0 000 2z" clip-rule="evenodd"></path></svg>Create</a>
  </div>
  <div v-if="categories === false" class="container mx-auto md:w-7/12  flex flex-col items-center">
