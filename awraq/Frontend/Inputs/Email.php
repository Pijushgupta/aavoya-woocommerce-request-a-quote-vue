...
class Email {
	public static function create($formInput, $key, $id) {
		...
		$form .= '<input type="email" id="' . esc_attr($id . $formInput['name'] . $key) . '" name="' . esc_attr($id . $formInput['name'] . $key) . '"  placeholder="' . __(sanitize_text_field($formInput['data']['placeholder']), AWRAQ_TEXT_DOMAIN) . '" ' . $required . '/>';
		$form .= '</div></div>';
		return $form;
	}
}
