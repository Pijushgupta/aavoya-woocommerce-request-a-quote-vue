...
class Text {
	public static function create($formInput, $key, $id) {
		...
		$form .= '<input id="' . esc_attr($id . $formInput['name'] . $key) . '" type="text" name="' . esc_attr($id . $formInput['name'] . $key) . '"  placeholder="' . __(sanitize_text_field($formInput['data']['placeholder']), AWRAQ_TEXT_DOMAIN) . '" ' . $required . ' class="w-full"/>';
		$form .= '</div></div>';
		return $form;
	}
}
