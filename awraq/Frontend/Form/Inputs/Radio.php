...
class Radio {
	...
	public static function create($formInput, $key, $id): string {
		...
		foreach ($formInput['data']['Options'] as $k => $v) {
			$form .= '<input type="radio" id="' . esc_attr($id . $formInput['name'] . $k) . '" name="' . esc_attr($id . $formInput['name'] . $key) . '" value="' . sanitize_text_field($v['value']) . '" style="margin-right:10px;"/>' . '<label for="' . esc_attr($id . $formInput['name'] . $k) . '">' . __(sanitize_text_field($v['name']), AWRAQ_TEXT_DOMAIN) . '</label><br>';
		}
		$form .= '</div></div>';
		return $form;
	}
