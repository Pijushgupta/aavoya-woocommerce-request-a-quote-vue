...
class Phone {
	public static function create($formInput, $key, $id): string {
		...
		$form .= '<input type="tel" id="' . esc_attr($id . $formInput['name'] . $key) . '" name="' . esc_attr($formInput['uniqueName']) . '"  placeholder="' . __(sanitize_text_field($formInput['data']['placeholder']), AWRAQ_TEXT_DOMAIN) . '" ' . $required . ' class="w-full"/>';
		$form .= '</div></div>';
		return $form;
	}
}
