...
class Phone {
	public static function create($formInput, $key, $id, $oldValueAsParam): string {
		...
		$form .= '<input type="tel" id="' . esc_attr($id . $formInput['uniqueName'] . $key) . '" name="' . esc_attr($formInput['uniqueName']) . '"  placeholder="' . __(sanitize_text_field($formInput['data']['placeholder']), AWRAQ_TEXT_DOMAIN) . '" ' . $required . ' class="aavoyaw-full" value="' . $value . '"/>';
		$form .= '</div></div>';
		return $form;
	}
}
